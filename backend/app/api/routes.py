...
from app.schemas.meeting import MeetingCreate, MeetingOut, TranscriptIn, MeetingAIOutputOut, ActionItemUpdate
...
async def upload_file(meeting_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    meeting = db.get(Meeting, meeting_id)
    if not meeting: raise HTTPException(404)
    os.makedirs(settings.upload_dir, exist_ok=True)
    file_path = os.path.join(settings.upload_dir, f"{meeting_id}_{file.filename}")
    with open(file_path, "wb") as f: f.write(await file.read())
    if file.content_type and file.content_type.startswith("audio"): meeting.audio_file_path = file_path
    else: meeting.video_file_path = file_path
    meeting.status = MeetingStatus.uploaded
    db.commit()
    return {"ok": True, "path": file_path}

@router.post("/meetings/{meeting_id}/transcript", response_model=MeetingOut)
def set_transcript(meeting_id: int, payload: TranscriptIn, db: Session = Depends(get_db)):
    meeting = db.get(Meeting, meeting_id)
    if not meeting: raise HTTPException(404)
...
async def process(meeting_id: int, db: Session = Depends(get_db)):
    meeting = db.get(Meeting, meeting_id)
    if not meeting: raise HTTPException(404)
    if not meeting.transcript_text: raise HTTPException(400, "Transcript required for MVP")
    llm = OpenRouterProvider(settings.openrouter_api_key or "")
    summarizer = MeetingSummarizer(llm, settings.openrouter_default_model)
    await process_meeting(meeting, db, summarizer, "openrouter", settings.openrouter_default_model)
    return {"ok": True}
...
def patch_action_item(action_item_id: int, payload: ActionItemUpdate, db: Session = Depends(get_db)):
...
def export_markdown(meeting_id: int, db: Session = Depends(get_db)):
    ...
    s = out.summary_json
    md = f"# {s['title']}\n\n## Executive Summary\n{s['executive_summary']}\n\n## Action Items\n"
    for a in s.get("action_items", []): md += f"- [ ] {a['task']} ({a.get('owner') or 'Unassigned'})\n"
    return md
